...
function SelectOrganizationBox() {
  const router = useRouter();

  const { filter } = useOrganizationList();
...
export default function OrganizationsSearchSection() {
  const [input, setInput] = useState<string>();
  const { keyword, fetchBoardList } = useOrganizationList();
  const router = useRouter();
  ...
  const goToCreateMemberPage = () => {
    router.push("/admin/organizations/create"); // 회원 생성 페이지로 이동
  };
